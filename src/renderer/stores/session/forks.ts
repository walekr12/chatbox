...
/**
 * Switch to a specific fork branch by list id.
 */
export async function switchForkToList(sessionId: string, forkMessageId: string, listId: string) {
  await chatStore.updateSessionWithMessages(sessionId, (session) => {
    if (!session) {
      throw new Error('Session not found')
    }
    const patch = buildSwitchForkToListPatch(session, forkMessageId, listId)
    if (!patch) {
      return session
    }
    return {
...
function buildSwitchForkPatch(
  ...
  const targetPosition =
    direction === 'next' ? (forkEntry.position + 1) % total : (forkEntry.position - 1 + total) % total

  const rootResult = switchForkInMessages(session.messages, forkEntry, forkMessageId, targetPosition, {
    dropEmptyCurrentBranch: true,
  })
  if (rootResult) {
...
function buildSwitchForkToListPatch(session: Session, forkMessageId: string, listId: string): Partial<Session> | null {
  const { messageForksHash } = session
  if (!messageForksHash) {
    return null
  }
  ...
  const targetPosition = forkEntry.lists.findIndex((list) => list.id === listId)
  if (targetPosition < 0 || targetPosition === forkEntry.position) {
    return null
  }
...
function switchForkInMessages(
  ...
  options?: { dropEmptyCurrentBranch?: boolean }
  ...
  if (targetPosition < 0 || targetPosition >= forkEntry.lists.length || targetPosition === currentPosition) {
    return null
  }
  ...
  if (options?.dropEmptyCurrentBranch && isCurrentBranchEmpty) {
    ...
    adjustedTargetPosition = targetPosition > currentPosition ? targetPosition - 1 : targetPosition
  }

  if (adjustedTargetPosition < 0 || adjustedTargetPosition >= updatedLists.length) {
    return null
  }
  const newPosition = adjustedTargetPosition
